	...
	private _syncTrigger(): void {
		...
		outputToObservable(popoverTrigger.triggerEvent)
			...
			.subscribe(() => {
				...
				// El estado real del popover es el que debe guiar el trigger
				const isOpen = this._popover()?.isOpen(); // o algún estado real si existe
				trigger.setExpandedState(isOpen !== undefined ? isOpen : false);
			});
