...
export class FrsPopoverContent {
	...
	protected readonly _frsClassPosition = computed(() => frs(variants({ position: this.position(), spacing: this.spacing() }), this.class()));
	protected readonly _frsClass = computed(() => frs('block w-full min-h-fit text-sm border rounded-md shadow-sm bg-background', this.class()));
}

const variants = cva('absolute z-50 w-full', {
	variants: {
		position: {
