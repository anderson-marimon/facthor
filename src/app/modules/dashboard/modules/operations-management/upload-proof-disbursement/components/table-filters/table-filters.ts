...
const OPERATION_STATUSES = [
	{ label: 'Desembolso', value: 13 },
	{ label: 'Reserva', value: 19 },
];

const SEARCH_OPTIONS = ['OrderNumber', 'LegalName', 'Tradename', 'IdentitificationNumber'];

@Component({
	selector: 'upload-proof-disbursement-table-filters',
...
export class UploadProofDisbursementTableFilters {
	...
	protected readonly _orderStatuses = OPERATION_STATUSES;
	...
	protected readonly _searchOrderStatusesControl = this._formBuilder.control<TSelectOption[]>([this._orderStatuses[0]]);
	...
	protected _onClickResetFilters(): void {
		...
		if (this.roleExecution() === this._eRoleExecution.FINANCIER) {
			this._searchOrderStatusesControl.setValue([this._orderStatuses[0]]);
		}
	}
}
