...
import { TApiGetPayerOperationsHistoryQuerySignalParams } from '@dashboard/modules/operation-history-management/payer-history/api/get-payer-operations-history';
...
@Component({
	selector: 'payer-history-table-filters',
...
export class PayerHistoryTableFilters {
	public readonly roleExecution = input(0);
	public readonly callback = input<() => void>();
	public readonly goBackToSideBar = input(false);
	public readonly isLoadingApiGetPayerOperationsHistory = input(false);
	public readonly filterFunction = input<(queryFilters: Partial<Omit<TApiGetPayerOperationsHistoryQuerySignalParams, 'Size'>>) => void>();
	...
	private _addSubscriptions(): void {
		this._searchPerDateRangeControl.valueChanges
			...
			.subscribe((rangeDates) => {
				this._getPayerOperationsHistoryByFilters({
	...
	private _getPayerOperationsHistoryByFilters(args: Partial<Omit<TApiGetActiveOperationsListQueryParams, 'Size'>>): void {
	...
	protected _onClickResetFilters(): void {
		if (this.isLoadingApiGetPayerOperationsHistory()) return;
