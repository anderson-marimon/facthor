...
@Component({
	selector: 'financier-history-table-filters',
...
export class FinancierHistoryTableFilters {
	public readonly roleExecution = input(0);
	public readonly callback = input<() => void>();
	public readonly goBackToSideBar = input(false);
	public readonly isLoadingApiGetFinancierOperationsHistory = input(false);
	public readonly filterFunction = input<(queryFilters: Partial<Omit<TApiGetFinancierOperationsHistoryQuerySignalParams, 'Size'>>) => void>();
	...
	private _addSubscriptions(): void {
		this._searchPerDateRangeControl.valueChanges
			...
			.subscribe((rangeDates) => {
				this._getFinancierOperationsHistoryByFilters({
	...
	private _getFinancierOperationsHistoryByFilters(args: Partial<Omit<TApiGetFinancierOperationsHistoryQueryParams, 'Size'>>): void {
	...
	protected _onClickResetFilters(): void {
		if (this.isLoadingApiGetFinancierOperationsHistory()) return;
