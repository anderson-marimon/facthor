...
export class NegotiationParametersTableFilters {
	...
	private _addSubscriptions(): void {
		this._searchPerDateControl.valueChanges
			...
			.subscribe((date) => {
				this._getNegotiationParametersByFilters({
		...
		this._searchInputControl.valueChanges
			...
			.subscribe(([inputValue, selectedOptions]) => {
				...
				const selectedSearchOption = this._searchOptions[selectedOptionValue];
				const optionsToFilter = this._searchOptions.reduce(
					(acc, option) => {
	...
	private _getNegotiationParametersByFilters(args: Partial<Omit<TApiGetNegotiationParametersQueryParams, 'Size'>>): void {
	...
	protected _onClickResetFilters(): void {
		if (this.isLoadingApiGetNegotiationParameters()) return;
